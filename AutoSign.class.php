...
class AutoSign
{
    ...
    public function __construct($user_name, $password, $count = 20, $is_mail = true)
    {
        ...
        $msg .= "【运行次数】：" . ($i + 1) . " 次 <br /> ";
        $msg .= "【开始时间】：" . date("Y-m-d H:i:s", $begin_time)."<br />";
        $msg .= "【运行时间】：" . ($end_time - $begin_time) . "秒<br />";
...
//        echo $msg;
     ...
     * @param string $cookie cookie文件的地址，默认为$this->_code_cookie
     * @return mixed
     */
    public function post($url, $data, $cookie = null)
    {
        //$cookie赋初值
        !$cookie && $cookie = $this->_code_cookie;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
